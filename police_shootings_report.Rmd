...
citycrime   <- fread("citycrime2_250000.csv")
citycrime <- citycrime %>% 
    select(V1,V5,V11)
citycrime$V1 <- gsub("([A-Za-z]+).*", "\\1", citycrime$V1)
...
adjust_variables_values <- function(dataframe){
    ...
    mutate(threat = ifelse(str_detect(threat, "attack"), "clear_attack", "no_clear_attack")) %>%
...
p19 <- shootings %>%
    mutate(armed2   = ifelse(str_detect(armed, "unarmed"), "unarmed", "armed"))  %>%
    ggplot() + 
    geom_bar(mapping = aes(x=camera, fill=armed2), position = "fill", stat="count") +
        theme_gray() +
        xlab("") +
        ylab("") +
...
```{r, messge=FALSE, warning=FALSE, include=FALSE}
p12 <- shootings %>% 
...
p15 <- shootings %>%
    ...
    ggplot(aes(x = reorder(state, ratio2), y = ratio2)) +
        ...
        xlab("") +
        ylab("") +
        ggtitle("P17: Ratio between black victims per 100.000 black residents and white victims per 100.000 residents")+ 
...
<p align="justify">
We also can take a very limited look at crime rates. Generally one would expect that violent shooting occur more often in cities with high crime rates. Let's take a look at some cities in the US with at least 250.000 residents were shootings occured. Interestingly it seems that only non-violent crime rates matter, though, I need much more observations to prove this hint. Besides, the plot is inflicted by sample bias due too choosing only cities with a high number of victims. Anyways, a possible next step might be to test weather or not non-violent crime rates account for a potential race bias. Note, that this does not mean that racism isn't a potential cause for those crime rates. However, I lack data to test the role of race based crime rates properly.  
</p>
...
p25 <- shootings %>%
    ...
    inner_join(city_pop) %>%
    mutate(shot_100000 = (shot/pop)*100000) %>%
    inner_join(citycrime) %>%
    select(city, shot_100000, vio_crime, non_vio_crime) %>%
    gather(type, value, -city, -shot_100000) %>%
        ggplot(aes(x=shot_100000, y=value, color= type)) +
            geom_point(shape=1) +    # Use hollow circles
            geom_smooth(method=lm) + 
            ylab("") +
            xlab("") +
            ggtitle("Influence of crime rates on fatal police shootings") +
            theme(text=element_text(size=7, family="Comic Sans MS"))
```
...
map <- map + labs(title="P19: Fatal Police Shootings (Jan. 2015-Feb. 2017)")
...
<p align="justify">
Lastly, I also wanted to show the value of choropleth maps if you want to compare values across regions. Plot 19 is a simple and clean choropleth map showing number of shootings across the different states of the US.  
</p>
...
aggr(shootings[3:13], prop = T, numbers = F)
```
...
get_data <- function(){
    ...
    shootings <- shootings %>%
        ...
        inner_join(state_rural) %>%
        # drop useless variables 
        select(-name, -Nonmetropolitan,-Total,-manner) %>%
        # rename state
        rename(state = Location) %>%
        # unify variable names
        clean_names()
