...
# get shootings per state for p19
...
# get shootings on unarmed percentage per state for p20
...
# set levels for plot 19 later on 
...
# set levels for plot 20 later on 
...
map <- map + theme_bw()
map <- map + geom_polygon(aes(x = long, y = lat, 
                          group = group,fill = factor(victims)),
...
map <- map + scale_fill_manual(name="Number of Incidents", values = pal(12) )
map <- map + guides(fill = guide_legend(override.aes = list(colour = NULL)))
map <- map + labs(title="P19: Lethal Police Shootings in US states (Jan. 2015-Feb. 2017)")
map <- map + theme(text = element_text(size = 8, family = "Comic Sans MS"))
...
# add a colors and a legend and title
map2 <- map2 + scale_fill_manual(name="Percent of Unarmed Victims ", values = pal(12) )
...
map2 <- map2 + theme(text = element_text(size = 8, family = "Comic Sans MS"))
...
map2 <- map2 + theme(axis.ticks = element_blank(),
                   ...
                   axis.title.y= element_blank())
# hide gridlines
map2 <- map2 + theme(panel.grid.minor=element_blank(),
                   panel.grid.major=element_blank())
```
...
<p align="justify">
If I would be forced to choose three plots, for example for a short blog article, I would choose plot 1, plot 19, and plot 20. Plot one to my mind clearly shows that there is an issue of great extend in the first place and that it does persist for at least two year. Plot 19 adds a geographical component and illustrates that the issue is not regional but rather nation-wide. Lastly, plot 20 shows that victims are often unarmed indicating that the police's use of force often may be excessive. I'm not sure about using the race related plots since people may jump to conclusions, though if I would use any then plot 16 because it shows that blacks are unproportional often victims (without explaining it) or plot 18 because it is the most reasonable yet weak indicator for a potential problem with racist police officers. Since, all plots are polished (esspecially 1,19, and 20) and have been discussed I do not see the point in just copy pasting them.  
</p>
